...
function Result({ setLoading }) {
  ...
  const filter = Result.filter(item => !item?.correct )


  const results = state?.result || filter || [];
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-6">
      <div className="bg-white shadow-md rounded-lg w-full max-w-4xl p-8">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-blue-500 mb-2">Wrong Answers only </h1>
          <p className="text-gray-700">
            Questions: <span className="font-semibold">{filter?.length}</span> 
          </p>
        </div>
